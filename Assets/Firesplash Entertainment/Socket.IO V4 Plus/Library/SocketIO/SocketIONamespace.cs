        ...
        public void On<T>(string eventName, UnityAction<T> callback)
        {
            ...
            registeredUnityEventHandlers[eventName].AddListener((e) => {
                ...
                catch (Exception ex)
                {
                    ...
                    //more details
                    UnityEngine.Debug.LogWarning("[Socket.IO@" + namespacePath + "] Not invoking event callback for " + eventName + " as it expects exactly one payload of type " + typeof(T).Name + " but we have some payload of type " + e.payloads[0].GetType().Name + ". If you need a universal method, use the non-generic callback with one parameter.");
                    return;
                }
            });
        }
